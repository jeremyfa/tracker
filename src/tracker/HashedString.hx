    ...
    public static function decode(encoded:String):String {
        ...
        while (i < len) {
            ...
            if (sectionLen == null || sectionLen <= 0) {
                Tracker.backend.warning('Failed to parse all encoded string: invalid section length');
            ...
            if (section == null) {
                Tracker.backend.warning('Failed to parse all encoded string: null section');
                _lastDecodeIncomplete = true;
                break;
            }
            if (Md5.encode(section) != hash) {
                Tracker.backend.warning('Failed to parse all encoded string: section hash mismatch');
                _lastDecodeIncomplete = true;
                break;
            }
            i += sectionLen;
        ...
        if (result != null) {
            ...
            Tracker.backend.error('Invalid encoded string');
            _lastDecodeIncomplete = true;
            return null;
        }
